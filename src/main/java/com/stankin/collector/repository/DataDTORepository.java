...
public interface DataDTORepository extends Repository<DataDTO, Long> {

    @Query("SELECT id, device_id, value::text, created FROM values WHERE device_id = :device_id\n" +
            "ORDER BY created DESC LIMIT :rows")
    List<DataDTO> findByDeviceIdWithRows(@Param("device_id") Long deviceId, @Param("rows") Long rows);
}
